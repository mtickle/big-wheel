...
const Scoreboard = memo(({ players, playerScores, bank, turn, leader, gameState, bonusEligible = [] }) => {
  return (
    <div className="bg-slate-800/40 p-5 rounded-2xl border-2 border-slate-700 flex flex-col h-87.5 w-full">
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex flex-col">
                  <span className={`font-bold transition-colors ${isActive ? "text-yellow-400" : isEligible ? "text-green-400" : "text-slate-300"}`}>
                    {p.name}
                  </span>

                  {/* ✨ BONUS ELIGIBILITY BADGE */}
                  {isEligible && !isActive && (
                    <span className="text-[9px] font-black text-yellow-500 mt-0.5 tracking-widest animate-pulse">
                      ★ BONUS QUALIFIED
                    </span>
                  )}
                  ...
                  {/* 💰 THE BANK DISPLAY */}
                  {hasMoney && (
                    <span className="text-[10px] font-black text-green-400 mt-1 tracking-wider">
                ...
                <div className="text-right">
                  <span className={`text-2xl font-mono font-black block leading-none ${isTheDollar ? "text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" :
      ...
      <div className={`mt-6 p-3 rounded-xl border-2 transition-all duration-700 ${gameState === "finished"
        ...
        <p className="text-[10px] font-black uppercase text-slate-500 mb-0.5 tracking-widest">
          {gameState === "finished" ? "🏆 Showcase Winner" :
            gameState === "spin_off" ? "⚖️ Tie-Breaker" :
